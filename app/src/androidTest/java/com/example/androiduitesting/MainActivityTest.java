...
// You can also use anything() in place of is(instanceOf(String.class))
    ...
    // Added androidTestImplementation ("androidx.test.espresso:espresso-intents:3.6.1") to gradles
    @Test
    public void testActivitySwitch() {
        init();
        // Add Edmonton city to list
        onView(withId(R.id.button_add)).perform(click());
        onView(withId(R.id.editText_name)).perform(ViewActions.typeText("Edmonton"));
        onView(withId(R.id.button_confirm)).perform(click());
    ...
    public void testShowActivityCity() {
        ...
        // Click on Edmonton city from listview
        onData(is(instanceOf(String.class))).inAdapterView(withId(R.id.city_list)).atPosition(0).perform(click());
        // Test if textview shows Edmonton in ShowActivity
        onView(withId(R.id.city_display)).check(matches(withText("Edmonton")));
    }
